# coding: utf-8
# Adapted from MVSEP-MDX23-Colab v2.5 by jarredou
# https://github.com/jarredou/MVSEP-MDX23-Colab_v2
# Originally from ZFTurbo/MVSEP-MDX23-music-separation-model
# https://github.com/ZFTurbo/MVSEP-MDX23-music-separation-model
# Modifications: output file naming (song_base), web app integration
            ...
            while i < mix.shape[1]:
                ...
                if len(batch_data) >= batch_size or (i >= mix.shape[1]):
                    ...
                    for j in range(len(batch_locations)):
                        ...
                        out_l = min(l, x[j].shape[-1])  # model may output fewer samples than C (e.g. 6-stem on GPU)
                        result[..., start:start+out_l] += x[j][..., :out_l].cpu() * window[..., :out_l]
                        counter[..., start:start+out_l] += window[..., :out_l]
...
def demix_new_multi_wrapper(mix, device, model, config, dim_t=256, bigshifts=1):
    """Like demix_new_wrapper but returns all stems as a dict {stem_name: array}."""
    if bigshifts <= 0:
        bigshifts = 1
    ...
    # Accumulate per-stem results across shifts
    results = {}  # stem_name -> list of arrays

    for shift in tqdm(shifts, position=0):
        shifted_mix = np.concatenate((mix[:, -shift:], mix[:, :-shift]), axis=-1)
        sources = demix_new(model, shifted_mix, device, config, dim_t=dim_t)
        for stem_name, arr in sources.items():
            unshifted = np.concatenate((arr[..., shift:], arr[..., :shift]), axis=-1)
            results.setdefault(stem_name, []).append(unshifted)

    return {stem: np.mean(arrs, axis=0) for stem, arrs in results.items()}
...
class EnsembleDemucsMDXMusicSeparationModel:
    ...
    def __init__(self, options):
        ...
        # Preloading ensemble models (if not vocals only and not using 6-stem)
        if not options.get('vocals_only', False) and not options.get('use_BSRoformer6Stem', False):
            self.models = []
    ...
    def instruments(self):
        if self.options.get('use_BSRoformer6Stem'):
            return ['bass', 'drums', 'other', 'vocals', 'guitar', 'piano']
        elif not self.options.get('vocals_only', False):
            return ['bass', 'drums', 'other', 'vocals', 'piano']
        else:
            return ['vocals']
    ...
    def separate_music_file(self, mixed_sound_array, sample_rate, current_file_number=0, total_files=0):
        ...
        # ── 6-stem BS-Roformer path (replaces vocal ensemble + Demucs) ──────
        if options.get('use_BSRoformer6Stem'):
            print('Processing with BSRoformer6Stem...')
            self.initialize_model_if_needed('BSRoformer6Stem', self.options)
            stems = demix_new_multi_wrapper(
                mixed_sound_array.T, self.device,
                self.model_bs6, self.config_bs6,
                dim_t=1101, bigshifts=options['BigShifts'],
            )
            for stem_name, arr in stems.items():
                separated_music_arrays[stem_name] = match_array_shapes(arr, mixed_sound_array.T).T
                output_sample_rates[stem_name] = sample_rate
            separated_music_arrays['instrum'] = mixed_sound_array - separated_music_arrays['vocals']
            output_sample_rates['instrum'] = sample_rate
            if not options['large_gpu']:
                print('Unloading BSRoformer6Stem from memory')
                self.model_bs6.cpu()
                del self.model_bs6
        ...
        if options['vocals_only'] is False:
            ...
            # Save piano/guitar before folding into other
            piano_stem = out[5].copy()  # shape: (channels, time)
            ...
            # piano from htdemucs_6s (only model with a dedicated piano stem)
            separated_music_arrays['piano'] = piano_stem.T
            output_sample_rates['piano'] = sample_rate
...
def predict_with_model(options):
    ...
    for i, input_audio in enumerate(options['input_audio']):
        ...
        raw = os.path.splitext(os.path.basename(input_audio))[0]
        song_base = re.sub(r'[^\w\s-]', '', raw).strip().lower()
        song_base = re.sub(r'[\s-]+', '_', song_base)

        for instrum in model.instruments:
            output_name = song_base + '_{}.{}'.format(instrum, output_extension)
        ...
        if options['vocals_only'] is False:
            ...
            output_name = song_base + '_{}.{}'.format('instrum2', output_extension)
...
if __name__ == '__main__':
    ...
    m.add_argument("--use_BSRoformer6Stem", action='store_true',
                   help="Use 6-stem BS-Roformer (vocals/bass/drums/guitar/piano/other) — replaces vocal ensemble + Demucs")
